...
def _request():
    return {
        ...
        "queryable_post_end": "2026-08-17T12:00:00+00:00",
...
def test_repricing_keeps_distinct_sale_days_from_same_multi_quantity_listing(tmp_path):
    ...
    with path.open("w",newline="",encoding="utf-8") as handle:
        ...
        for sold_date,price,item_id in rows:
            writer.writerow({
                "Item Title":"2026 Topps Chrome Player One #10",
                "Sold Price":f"${price}",
                "Sold Date":sold_date,
                "Item ID":item_id,
                "Currency":"USD",
    ...
    assert result["verification"]["pre_count"]==3
    assert result["verification"]["post_count"]==3
