...
def test_new_sale_explains_repricing():
    ...
    assert "accepted_sales_changed" in delta["change_reasons"]
    assert "latest_sale_changed" in delta["change_reasons"]
    assert delta["unexplained_repricing"] is False
    assert delta["reconstruction_health_failure"] is False
...
def test_supply_or_confidence_change_is_attributed():
    ...
    assert "active_supply_changed" in delta["change_reasons"]
    assert "confidence_changed" in delta["change_reasons"]
    assert delta["reconstruction_health_failure"] is False
