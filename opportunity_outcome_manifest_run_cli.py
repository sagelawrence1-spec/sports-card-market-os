"""CLI for executing mature authoritative outcome manifest items."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Execute all mature items from an authoritative outcome manifest."
    )
    parser.add_argument("--manifest", required=True)
    parser.add_argument("--assets", required=True, help="JSON object keyed by canonical card_id")
    parser.add_argument("--export-dir", required=True)
    parser.add_argument("--min-forward-comps", type=int, default=3)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        artifact = run_authoritative_outcome_manifest(
            manifest,
            assets=assets,
            export_dir=args.export_dir,
            min_forward_comps=args.min_forward_comps,
        )
        _write_json(artifact, args.output)
    except (OSError, json.JSONDecodeError, KeyError, TypeError, ValueError) as exc:
        sys.stderr.write(f"opportunity-outcome-run error: {exc}\n")
        return 2
    return 0
