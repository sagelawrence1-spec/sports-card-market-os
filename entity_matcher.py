...
LOT_WORDS={"lot","bundle","set of","cards"}
...
def contains_phrase(title, phrase):
    return norm(phrase) in norm(title) if phrase else True
...
class SportsCardEntityMatcher:
    ...
    def match(self, asset:Dict[str,Any], title:str) -> MatchDecision:
        ...
        # Year is highly discriminative for modern sports cards.
        if year:
            if year in t: score += 10; diag["year_match"]=1
            else:
                shown_years=re.findall(r"\b(?:19|20)\d{2}(?:-\d{2})?\b",t)
                if shown_years:
                    return MatchDecision(False,20.0,"wrong_year",{**diag,"explicit_years":shown_years})
                score -= 9; diag["year_match"]=0

        # Card number: normalize #136 / no. 136 / 136.
        if cardnum:
            pat=rf"(?<![a-z0-9])(?:#|no\.?\s*)?{re.escape(cardnum)}(?![a-z0-9])"
            card_match=bool(re.search(pat,t))
            if card_match:
                score += 9; diag["card_number_match"]=1
            else:
                diag["card_number_match"]=0
                # A title showing a different explicit card number is a variant mismatch, not weak evidence.
                explicit_nums=re.findall(r"(?:#|no\.?\s*)([a-z0-9-]+)",t)
                # Also catch set-style card numbers such as HMT1 vs HMT32 even when # is omitted.
        ...
        # Set/manufacturer token overlap.
        ...
        # Grade must match exactly for graded assets; raw listings are rejected.
        if grade_company:
            ...
            if exact: score += 12
                ...
                # Another grading company is a hard variant mismatch.
                other=GRADE_COMPANIES & title_tokens
                if other: return MatchDecision(False,25.0,"wrong_grading_company",{**diag,"other_grader":sorted(other)})
        ...
        if parallel and parallel not in {"base","base card"}:
            ...
            # A named parallel is a different collectible from the target base card.
        ...
        # Serial denominator evidence, when target is numbered.
        if serial and serial not in {"0","None"}:
            if re.search(rf"/\s*{re.escape(serial)}\b",t): score += 7; diag["serial_denominator_match"]=1
            else: score -= 12; diag["serial_denominator_match"]=0

        # Multi-card lots are not clean single-card comps unless title clearly means population wording.
...
def build_ebay_query(asset:Dict[str,Any]) -> str:
    ...
    if p and p.lower() not in {"base","base card"}: parts.append(p)
    if int(float(asset.get("autograph") or 0)): parts.append("auto")
    gc=str(asset.get("grade_company") or "").strip()
    gr=str(asset.get("grade") or "").replace(".0","").strip()
    if gc: parts.append(gc)
    if gc and gr: parts.append(gr)
    return " ".join(parts)
