...
def exposure_headroom(
    *,
    ...
    dimensions = {
        ...
        "correlated_bucket": (
            "correlated_bucket",
            candidate.correlated_bucket,
            policy.max_correlated_bucket_pct,
        ),
    }
...
def apply_exposure_caps(
    ...
    exposure_by_card: Mapping[str, CandidateExposure],
    *,
    portfolio_value: float,
    positions: list[PositionExposure] | None = None,
    policy: ExposurePolicy | None = None,
) -> list[dict[str, Any]]:
    """Clamp approved allocations to portfolio concentration headroom.

    This layer never increases an upstream allocation. Missing exposure metadata
    fails closed to zero because an unclassified position cannot be concentration-safe.
    """
    policy = policy or ExposurePolicy()
    ...
    for raw in allocations:
        ...
        allowed = min(
            requested,
            *(dimension["headroom"] for dimension in headroom.values()),
        )

        blockers = [
            f"{name}_cap_reached"
            for name, dimension in headroom.items()
            if dimension["headroom"] <= 0
        ]
